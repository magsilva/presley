...
public class MensagemAba extends ViewPart {
	...
	private void initComponents(Composite parent)
	{
		...
		logout.addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent e) {
				boolean answer  = MessageDialog.openQuestion( incluirProblema.getShell(),
			...
			public void mouseDown(MouseEvent e) {
				...
				if (excluir) {
					...
					try {
						...
						preenchelistaProblemasEnviados( );
					} catch (ProblemaInexistenteException p) {
						p.printStackTrace();
					}
		...
		encerrarProblema.addMouseListener(new MouseListener(){
			...
			public void mouseDown(MouseEvent e) {
				if ((treeProblemasEnviados.getSelectionCount() == 0) || (treeProblemasEnviados.getSelection()[0].getData() instanceof Solucao)){
					MessageDialog.openInformation(validarSolucao.getShell(),
							"Informação", "O Problema deve ser selecionado antes de ser Concluido");
				} else {
					Problema problema = (Problema) treeProblemasEnviados.getSelection()[0].getData();
		...
		mensagemRecebida.setBackground(new Color(mensagemRecebida.getDisplay(),247,193,193)); 

		treeProblemasRecebidos = new Tree(parentComposite, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL );
			...
			public void mouseDoubleClick(MouseEvent e) {
				if (treeProblemasRecebidos.getSelectionCount() != 0){
					if (treeProblemasRecebidos.getSelection()[0].getData() instanceof Solucao){
						...
						if ((solucao.getSolucaoResposta()== null) && (!solucao.getSolucaoResposta().equals("")))
							runEnviaRespostaWizardAction(solucao.getProblema(), solucao);
					} else if (treeProblemasRecebidos.getSelection()[0].getData() instanceof Problema){
		...
		lblGrupoBotoes3.setBackground(new Color(lblGrupoBotoes3.getDisplay(),204,204,204));
		...
		for (Iterator<Solucao> iterator = solucoes.iterator(); iterator.hasNext();) {
			...
			if ( problemaEnviado ||
				...
				if ( solucao.getSolucaoResposta() != null ){
					...
					preecheTreeItemSolucao( solucaoResposta, item[numItem], problemaEnviado);
				}
				numItem++;
			}
	...
	private void runEnviaRespostaWizardAction(Problema problema, Solucao solucaoOrigem){
