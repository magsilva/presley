...
public class ServicoSolucaoImplDAOTest extends TestCase {
	...
	private void criarDesenvolvedores() {
		...
		assertTrue(sd.criarDesenvolvedor("amilcarsj@gmail.com", "Amilcar Soares", "Jo�o Pessoa", "000000"));
...
/*		
		...
		sp.cadastrarProblema(list.get(0).getId(), "Erro de classpath", dataDoRelato, "Stack trace do problema, n�o acha o classpath ...");
...
/*
  		assertTrue(ss.cadastrarSolucao("amilcarsj@gmail.com", plist.get(0).getId(), 
				dataDaProposta, "Isso provavelmente � erro no classpath, tente alter�-lo..."));
...
*/	
	...
	public void testListarSolucoesDoDesenvolvedor() {
		...
		assertTrue(list.get(0).getMensagem().equals("Isso provavelmente � erro no classpath, tente alter�-lo..."));
		
		if (list.get(0).getMensagem().equals("Isso provavelmente � erro no classpath, tente alter�-lo..."))
	...
	public void testAtualizarStatusDaSolucao() {
	...
	public void testeGetSolucao(){
		...
		assertTrue(s.getMensagem().equals("Isso provavelmente � erro no classpath, tente alter�-lo..."));
		assertTrue(s.getDesenvolvedor().getEmail().equals("amilcarsj@gmail.com"));
		assertTrue(s.getProblema().getMensagem().equals("Stack trace do problema, n�o acha o classpath ..."));
...
//		this.removerAtividade();
		...
		if (ss.removerSolucao(list.get(0).getId()))
			fail("essa solucao n�o devia existir mais no banco!");
